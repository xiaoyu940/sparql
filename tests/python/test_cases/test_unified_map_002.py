...
class TestMap002(TestCaseBase):
    """测试带表前缀变量映射回归测试: employees, departments, salaries"""
    ...
    def sparql_query(self) -> QueryResult:
        ...
        # 验证生成的 SQL 包含预期模式
        expected_patterns = ["department_name", "salary"]
        excluded_patterns = ["t0.dept"]  # 不应出现未映射的变量
...
if __name__ == '__main__':
    ...
    print(f"\n{'='*80}")
    print(f"测试: 带表前缀变量映射回归测试")
    print(f"{'='*80}\n")
    
    result = run_test_case(TestMap002, db_config)
