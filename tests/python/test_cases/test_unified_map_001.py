...
class TestMap001(TestCaseBase):
    """测试变量到列映射: departments"""
    ...
    def sparql_query(self) -> QueryResult:
        ...
        不应出现: GROUP BY deptName (变量名不应直接出现在GROUP BY中)
        """
        sparql = """
        ...
        # 验证生成的 SQL 包含预期模式
        expected_patterns = ["department_name"]
        excluded_patterns = ["GROUP BY deptName"]  # 变量名不应直接出现在GROUP BY
...
if __name__ == '__main__':
    ...
    result = run_test_case(TestMap001, db_config)
