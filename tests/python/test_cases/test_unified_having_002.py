...
验证 SPARQL HAVING多条件查询是否正确翻译为 SQL
"""
...
class TestHaving002(TestCaseBase):
    """测试HAVING多条件: employees, departments, salaries"""
    
    def sparql_query(self) -> QueryResult:
        """
        SPARQL 查询: 获取平均薪资超过50000且员工数超过5的部门
        
        对应 SQL:
        SELECT dep.department_name AS deptName, AVG(emp.salary) AS avgSalary, COUNT(emp.employee_id) AS empCount
        ...
        HAVING AVG(emp.salary) > 50000
          ORDER BY dep.department_name AND COUNT(emp.employee_id) > 5
          ORDER BY dep.department_name
        """
        sparql = """
        ...
        } GROUP BY ?deptName HAVING (AVG(?salary) > 50000 && COUNT(?emp) > 5) ORDER BY ?deptName
        """
        ...
        # 验证生成的 SQL 包含预期模式
        expected_patterns = ["HAVING", "AVG", "COUNT"]
        excluded_patterns = []
    ...
    def sql_query(self) -> QueryResult:
        ...
        INNER JOIN salaries AS sal ON emp.employee_id = sal.employee_id
        GROUP BY dep.department_name
        HAVING AVG(emp.salary) > 50000 AND COUNT(emp.employee_id) > 5
        ORDER BY dep.department_name
        """
...
if __name__ == '__main__':
    ...
    print(f"\n{'='*80}")
    print(f"测试: HAVING多条件")
    print(f"{'='*80}\n")
    
    result = run_test_case(TestHaving002, db_config)
