...
验证 SPARQL 三表 JOIN 聚合查询是否正确翻译为 SQL
"""
...
class TestJoin002(TestCaseBase):
    """测试三表JOIN聚合: employees, departments, salaries"""
    
    def sparql_query(self) -> QueryResult:
        """
        SPARQL 查询: 获取部门名称和薪资统计
        
        对应 SQL:
        SELECT dep.department_name AS deptName, AVG(emp.salary) AS avgSalary, SUM(emp.salary) AS totalSalary
        ...
        # 验证生成的 SQL 包含预期模式
        expected_patterns = ["AVG", "SUM", "salary"]
        excluded_patterns = []
...
if __name__ == '__main__':
    ...
    result = run_test_case(TestJoin002, db_config)
