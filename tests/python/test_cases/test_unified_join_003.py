...
验证 SPARQL JOIN 条件是否正确翻译，避免错误的 JOIN 条件
"""
...
class TestJoin003(TestCaseBase):
    """测试JOIN条件回归测试: employees 和 departments"""
    
    def sparql_query(self) -> QueryResult:
        """
        SPARQL 查询: 获取部门名称和员工数量
        ...
        不应出现: department_id = employee_id
        """
        sparql = """
        ...
        # 验证生成的 SQL 包含预期模式
        expected_patterns = ["INNER JOIN", "department_id"]
        excluded_patterns = ["department_id = employee_id"]  # 错误的JOIN条件
...
if __name__ == '__main__':
    ...
    print(f"\n{'='*80}")
    print(f"测试: JOIN条件回归测试")
    print(f"{'='*80}\n")
    
    result = run_test_case(TestJoin003, db_config)
