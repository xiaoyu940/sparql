...
CREATE TABLE ai_interaction_logs (
    ...
    response_text TEXT,
    token_usage INT,
    user_feedback TEXT CHECK (user_feedback IN ('Helpful', 'Not_Helpful', 'Corrected')),
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE planning_records (
    plan_id INT PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY,
    project_id INT NOT NULL,
...
CREATE TABLE requirements (
    req_id INT PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY,
    project_id INT NOT NULL,
    ...
    source VARCHAR(100),
    status TEXT CHECK (status IN ('Draft', 'Reviewed', 'Approved', 'Changed')),
...
CREATE TABLE design_artifacts (
    design_id INT PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY,
    project_id INT NOT NULL,
    design_type TEXT CHECK (design_type IN ('Architecture', 'Database', 'UI_UX', 'API_Spec')),
...
CREATE TABLE dev_tasks (
    ...
    status TEXT CHECK (status IN ('To_Do', 'In_Progress', 'Reviewing', 'Done')),
...
CREATE TABLE test_cases (
    case_id INT PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY,
    project_id INT NOT NULL,
    req_id INT,
...
CREATE TABLE bug_reports (
    bug_id INT PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY,
    case_id INT,
    project_id INT NOT NULL,
    title VARCHAR(255),
    ai_suggested_fix TEXT, -- AI 给出的修复建议代码
    severity TEXT CHECK (severity IN ('Minor', 'Major', 'Critical', 'Blocker')),
    status TEXT CHECK (status IN ('New', 'Open', 'Fixed', 'Verified', 'Closed')),
    reporter_id INT,
...
CREATE TABLE deployments (
    deploy_id INT PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY,
    project_id INT NOT NULL,
    version_tag VARCHAR(50),
    environment TEXT CHECK (environment IN ('Staging', 'UAT', 'Production')),
...
CREATE TABLE maintenance_logs (
    log_id INT PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY,
    project_id INT NOT NULL,
    issue_type TEXT CHECK (issue_type IN ('Server_Down', 'Performance_Issue', 'Security_Patch', 'User_Request')),
    description TEXT,
...
CREATE TABLE phase_transitions (
    transition_id INT PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY,
    project_id INT NOT NULL,
    from_phase_id INT,
    to_phase_id INT,
    approved_by INT,
