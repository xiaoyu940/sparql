...
old_func = '''    fn parse_exists_filter_expr(filter: &str, core: &LogicNode) -> Option<Expr> {
...
new_func = '''    fn normalize_exists_filter_text(filter: &str) -> String {
...
if old_func not in text:
    raise SystemExit('old parse_exists block not found')
text = text.replace(old_func, new_func, 1)

old_loop = '        for filter_str in &parsed.filter_expressions {'
new_loop = '''        for filter_str in &parsed.filter_expressions {
            let upper = filter_str.trim().to_ascii_uppercase();
            if upper.starts_with("EXISTS") || upper.starts_with("NOT EXISTS") {
                continue;
            }'''
if old_loop not in text:
    raise SystemExit('old filter_str loop not found')
text = text.replace(old_loop, new_loop, 1)

p.write_text(text, encoding='utf-8')
print('patched ir_converter exists flow')